...
class LongitudinalPlanner:
  ...
  def update(self, sm):
    ...
    def conditional_experimental_mode():
      ...
      def road_curvature(lead, standstill):
      ...
      # Update conditions
      self.curve = road_curvature(lead, standstill)
      ...
      stop_light_detected = stop_sign_and_light(lead, standstill)
      ...
      status_value = 1 if overridden == 1 else 2 if overridden == 2 else 3 if signal else 4 if stop_light_detected else 5 if self.curve else 6 if speed and not lead else 7 if speed else 0
    ...
    if self.conditional_experimental_mode and carState.cruiseState.available:
