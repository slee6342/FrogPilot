  ...
  for (int i = 0; i < toggles.size(); i++) {
    ...
    if (key == "FrogTheme") {
      ...
      createSubControl(key, label, desc, icon, {
        ...
        {"ConditionalExperimentalModeSignal", "Experimental Mode With Turn Signal", "Activate 'Experimental Mode' whenever the turn signal is on to assist with turns."}
      });
      ...
      createSubControl(key, label, desc, icon, {
        ...
        {"UnlimitedLength", "'Unlimited' Length", "Extends the road UI out as far as the model can see."}
      });
